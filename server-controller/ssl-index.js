...
const app = express();
const PORT = 9002; // HTTPS Controller runs on port 9002
...
app.get('/health', (req, res) => {
  res.json({ 
    status: 'HTTPS Controller service running', 
...
httpsServer.listen(PORT, () => {
  console.log(`🚀 HTTPS Server Controller running on https://server.harryrose.dev:${PORT}`);
  console.log(`🔒 SSL Certificate loaded successfully`);
  console.log('📊 Available endpoints:');
  console.log(`   GET  https://server.harryrose.dev:${PORT}/health`);
  console.log(`   GET  https://server.harryrose.dev:${PORT}/server-status`);
  console.log(`   POST https://server.harryrose.dev:${PORT}/start-server`);
  console.log(`   POST https://server.harryrose.dev:${PORT}/stop-server`);
  console.log('This service manages the OTP server lifecycle with SSL');
});

// Handle server errors
httpsServer.on('error', (error) => {
  if (error.code === 'EACCES') {
    console.error(`❌ Permission denied on port ${PORT}. Try a different port.`);
...
process.on('SIGINT', async () => {
  console.log('\nShutting down HTTPS controller...');
  await serverManager.stopServer();
  process.exit(0);
});
